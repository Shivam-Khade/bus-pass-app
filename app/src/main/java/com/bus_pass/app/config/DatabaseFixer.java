...
public class DatabaseFixer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // 4. Check and Fix 'users' table (Add photo_url if missing)
        try {
            List<String> columns = jdbcTemplate.query(
                    "SHOW COLUMNS FROM users LIKE 'photo_url'",
                    (rs, rowNum) -> rs.getString(1));

            if (columns.isEmpty()) {
                System.out.println("Schema update: Adding 'photo_url' to 'users' table...");
                jdbcTemplate.execute("ALTER TABLE users ADD COLUMN photo_url VARCHAR(255) DEFAULT NULL");
                System.out.println("'photo_url' column added.");
            }
        } catch (Exception e) {
            System.err.println("Error checking/updating 'users' table: " + e.getMessage());
        }

        // 5. Check and Fix 'users' table (Add address if missing)
        try {
            List<String> columns = jdbcTemplate.query(
            ...
            if (columns.isEmpty()) {
                System.out.println("Schema update: Adding 'address' to 'users' table...");
                jdbcTemplate.execute("ALTER TABLE users ADD COLUMN address TEXT DEFAULT NULL");
                System.out.println("'address' column added.");
            }
        } catch (Exception e) {
            System.err.println("Error checking/updating 'users' table for address: " + e.getMessage());
        }
